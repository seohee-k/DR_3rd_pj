...
def main(args=None):
    ...
    try:
        from DSR_ROBOT2 import (
            ...
            check_force_condition, move_periodic, check_position_condition, task_compliance_ctrl,DR_MV_MOD_ABS,
    ...
    img_path = "/home/jin/Downloads/cat_image.png"
    ...
    Ready = posx([567.68, 112.22, 90, 118.03, 180, 118.08])
    ...
    for i, (x, y) in enumerate(robot_path[::10]):  # 간격 줄이기        
        if i == 0 :
            ...
            current_x = old_x - x
            current_y = old_y - y
            movel(posx([current_x, current_y, 0, 0, 0, 0]) ,vel=VELOCITY, acc=ACC, radius=0.0, ref=DR_BASE, mod=DR_MV_MOD_REL)
            old_x = x
            old_y = y
